...
def MVPA(subjects, mask_fname):
    ...
    cv_results_KNN = cvte(ds)
    KNN_orig = np.mean(cv_results_KNN)
    print 'Mean accuracy KNN_Orig=', KNN_orig
    print
    # test the significance
    t, p = stats.ttest_1samp(cv_results_KNN,.25)
    print 't_value=', t
    ...
    clf = kNN(k=1, dfx=one_minus_correlation, voting='majority')
    cvte = CrossValidation(clf, NFoldPartitioner(),errorfx=lambda p, t: np.mean(p == t))
    cv_new_KNN = cvte(ds_new)
    KNN_new = np.mean(cv_new_KNN)
    print 'Mean Accuracy KNN_new=', KNN_new
    # Test the results significance
    print
    t_new, p_new = stats.ttest_1samp(cv_new_KNN,.25)
    ...
    print
    # Test the results significance
    t_new_svm, p_new_svm= stats.ttest_1samp(cv_results_svm,.25)
    print
